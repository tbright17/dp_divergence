...
def dp_div(X1, X2):
    ...
    if m != n:
        print "CAUTION: The data sets have different numbers of samples: %6d, %6d\n" % (m, n)
    if ((dimY > n) or (dimX > m)):
        print "CAUTION: Are you sure the sampes are in rows and the features are in columns?"
    if (dimX != dimY):
    ...
    weightsOfEdges = squareform(pdist(dataX1X2, "minkowski", options["pNorm"]))
    ...
    while (currentMST < options["nTrees"]):
        ...
        listOfEdges = (listOfEdges[0]+nonzeroindex[0].tolist(),listOfEdges[1]+nonzeroindex[1].tolist())
